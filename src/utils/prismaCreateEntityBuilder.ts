export const prismaCreateEntityBuilder = (data: any, mapping?: any) => {
    ...
    console.log("prismaCreateEntityBuilder input data:", data, mapping)
    for (const key of Object.keys(data)) {
        ...
        if (Array.isArray(val)) {
            ...
            console.log("Array relation field:", key, val)
            // elementos objeto con id => connect by id
            if (typeof val[0] === "object") {
                if ("id" in val[0]) {
                    result[key] = { connect: val.map((it: any) => ({ [map.connectField || "id"]: it.id })) };
                ...
                result[key] = { connect: val.map((id: any) => ({ [map.connectField || "id"]: id })) };
            }
